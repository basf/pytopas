...
SIMPLE_DIR = Path(__file__).parent / "scale"
...
def test_scale_node(in_path: Path, json_path: Path, out_path: Path):
    "Test ScaleNode"
    text_in = in_path.read_text()[:-1]
    serialized = json.loads(json_path.read_text())
    text_out = out_path.read_text()[:-1]

    node = ast.ScaleNode.parse(text_in, parse_all=True)
    assert isinstance(node, ast.ScaleNode)
...
def test_scale_unserialize_fail():
    "Test ScaleNode"
    with pytest.raises(ReconstructException):
        ast.ScaleNode.unserialize([])
    with pytest.raises(ReconstructException):
        ast.ScaleNode.unserialize(["not this node", 1])
    with pytest.raises(ReconstructException):
        ast.ScaleNode.unserialize(["scale"])
    with pytest.raises(ReconstructException):
        ast.ScaleNode.unserialize(["scale", 1])
